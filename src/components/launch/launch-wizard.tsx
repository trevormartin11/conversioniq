...
const FALLBACK_TITLES = ["Owner / Founder", "CEO", "Marketing Manager", "Practice Manager", "Office Manager", "Director of Sales"];
...
export function LaunchWizard({
  ...
  const [problemOpts, setProblemOpts] = useState<string[]>([]);
  const [titleOpts, setTitleOpts] = useState<string[] | null>(null);
  ...
  const titleChips = titleOpts ?? FALLBACK_TITLES;
  ...
  async function doSuggestVerticals() {
    setError(null);
    setSuggesting("verticals");
    try {
      const r = await suggestVerticalsAction(problem.trim() || undefined);
      setIdeas(r.ideas);
    } catch {
  ...
  async function doSuggestProblems() {
    if (!vertical.trim()) return;
    setError(null);
    setSuggesting("problems");
    try {
      const r = await suggestProblemsAction(vertical.trim());
      setProblemOpts(r.problems);
    } catch {
      setError("Couldn't fetch suggestions — try again.");
    } finally {
      setSuggesting("");
  ...
  async function doSuggestTitles() {
    if (!vertical.trim()) return;
    setError(null);
    setSuggesting("titles");
    try {
      const r = await suggestTitlesAction(vertical.trim(), problem.trim() || undefined);
      setTitleOpts(r.titles);
    } catch {
      setError("Couldn't fetch suggestions — try again.");
    } finally {
      setSuggesting("");
  ...
  const chipCls = (on: boolean) => cn("chip", on ? "bg-brand/15 text-brand-300 ring-1 ring-inset ring-brand-500/25" : "bg-ink-700/60 text-slate-300 ring-1 ring-inset ring-white/10");
  const suggestBtn = "inline-flex items-center gap-1 text-[11px] font-medium text-brand-400 transition-colors hover:text-brand-300 disabled:opacity-40";
    ...
    <div className="space-y-5">
      ...
      <Card>
        <CardBody className="space-y-4">
          {/* 1 — vertical + the problem (bidirectional AI) */}
          {step === 0 && (
            <>
              <div>
                <p className="text-sm font-medium text-slate-100">1. Pick a vertical &amp; name the problem</p>
                <p className="text-xs text-slate-500">Let the AI suggest, or type your own — vertical and problem feed each other.</p>
              </div>
              <div>
                <div className="flex items-center justify-between gap-2">
                  <p className="text-xs font-medium text-slate-400">Vertical / ICP</p>
                  <button type="button" onClick={doSuggestVerticals} disabled={suggesting === "verticals"} className={suggestBtn}>
                    {suggesting === "verticals" ? <Loader2 className="h-3 w-3 animate-spin" /> : <Sparkles className="h-3 w-3" />}
                    {problem.trim() ? "Suggest verticals for this problem" : "Suggest verticals"}
                  </button>
                </div>
                ...
                {ideas.length === 0 && suggestions.length > 0 && (
                  <div className="mt-2 flex flex-wrap gap-2">
                    {suggestions.map((sug) => (
                      <button key={sug} type="button" onClick={() => setVertical(sug)} className={chipCls(vertical === sug)}>{sug}</button>
                    ))}
                  </div>
                )}
                {ideas.length > 0 && (
                  <div className="mt-2 grid gap-2 sm:grid-cols-2">
                    {ideas.map((idea) => (
                      <button
                        key={idea.vertical}
                        type="button"
                        onClick={() => { setVertical(idea.vertical); setProblem(idea.angle); }}
                        className={cn("rounded-lg border p-2.5 text-left transition-colors", vertical === idea.vertical ? "border-brand-500/40 bg-brand-600/10" : "border-ink-700 bg-ink-900/40 hover:border-ink-600")}
                      >
                        <div className="flex items-center justify-between gap-2">
                          <span className="text-sm font-medium text-slate-200">{idea.vertical}</span>
                          <span className="text-[10px] text-slate-500">fit {idea.fit}/10</span>
                        </div>
                        <p className="mt-0.5 text-[11px] text-slate-400">{idea.angle}</p>
                      </button>
                    ))}
                  </div>
                )}
              ...
              <div>
                <div className="flex items-center justify-between gap-2">
                  ...
                  <button type="button" onClick={doSuggestProblems} disabled={!vertical.trim() || suggesting === "problems"} className={suggestBtn}>
                    {suggesting === "problems" ? <Loader2 className="h-3 w-3 animate-spin" /> : <Sparkles className="h-3 w-3" />}
                    Suggest the problem
                  </button>
                </div>
                <textarea
                  ...
                  <div className="mt-2 space-y-1.5">
                    {problemOpts.map((p) => (
                      <button
                        ...
                        className={cn("block w-full rounded-lg border p-2 text-left text-xs transition-colors", problem === p ? "border-brand-500/40 bg-brand-600/10 text-slate-200" : "border-ink-700 bg-ink-900/40 text-slate-400 hover:border-ink-600")}
                      >
                        {p}
                      </button>
                    ))}
          ...
          {/* 2 — the people (AI role suggestions) */}
          {step === 1 && (
            <>
              <div>
                <p className="text-sm font-medium text-slate-100">2. Find the people</p>
                <p className="text-xs text-slate-500">
                  Who inside each {vertical || "company"} owns this problem and can say yes. We source → enrich → verify → dedupe → load (live once your data keys are connected).
                </p>
              </div>
              <div>
                <div className="flex items-center justify-between gap-2">
                  <p className="text-xs font-medium text-slate-400">Target titles / roles</p>
                  <button type="button" onClick={doSuggestTitles} disabled={!vertical.trim() || suggesting === "titles"} className={suggestBtn}>
                    {suggesting === "titles" ? <Loader2 className="h-3 w-3 animate-spin" /> : <Sparkles className="h-3 w-3" />}
                    Suggest roles
                  </button>
                </div>
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  {titleChips.map((t) => (
                    <button key={t} type="button" onClick={() => toggleTitle(t)} className={chipCls(selectedTitles.includes(t))}>{t}</button>
                  ))}
                </div>
                {titleOpts && <p className="mt-1 text-[11px] text-slate-500">AI-suggested for {vertical || "this vertical"} — tap to add, or type your own.</p>}
              </div>
              <div>
